...
public class ClearEditText extends EditText implements OnFocusChangeListener,
	...
	private void init(Context context) {
		...
		if (mClearDrawable == null) {
			...
			mClearDrawable = getResources().getDrawable(R.drawable.ic_mini_deletegrey);
		}
	...
	public void setIconLocation(boolean left, ImageView passwordIcon) {
		int offset = 8;
		if (left) {
			offset = 25;
		}
		...
		if (mClearDrawable == null) {
			...
			mClearDrawable = getResources().getDrawable(
					R.drawable.ic_mini_deletegrey);
		}

		mClearDrawable.setBounds(
				-mClearDrawable.getIntrinsicWidth()
						- Utils.dip2px(context, offset), 0,
				-Utils.dip2px(context, offset),
				mClearDrawable.getIntrinsicHeight());
		setClearIconVisible(false);
		// 设置焦点改变的监听
		setOnFocusChangeListener(this);
	...
	private void setPasswordIconVisible(boolean visible) {
		if (visible && passwordIcon != null) {
			...
			if (!visible && passwordIcon != null) {
				...
				/*passwordIcon
						.setImageResource(R.drawable.ic_mini_passwordvisible);*/
				passVisible = false;
		...
		if (event.getAction() == MotionEvent.ACTION_UP) {
			if (getCompoundDrawables()[2] != null) {
				...
				if (!isPassword) {
					...
					touchable = event.getX() > (getWidth()
							- getTotalPaddingRight()
							- mClearDrawable.getIntrinsicWidth() - Utils
								.dip2px(context, 25))
							&& (event.getX() < ((getWidth() - getPaddingRight()
									- mClearDrawable.getIntrinsicWidth() - Utils
										.dip2px(context, 25))));
	...
	public void onFocusChange(View v, boolean hasFocus) {
		...
		if (hasFocus) {
			...
			setPasswordIconVisible(getText().length() > 0);
		} else {
			setClearIconVisible(false);
	...
	public void onTextChanged(CharSequence s, int start, int count, int after) {
		if (hasFoucs) {
			...
			setPasswordIconVisible(s.length() > 0);
